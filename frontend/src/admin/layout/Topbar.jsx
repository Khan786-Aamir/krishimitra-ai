...
export const Topbar = ({
  ...
  const [notifications, setNotifications] = useState([
    { _id: '1', title: 'New Expert Registration', message: 'Dr. Anita Verma has requested verification credentials.', priority: 'high', read: false, createdAt: new Date().toISOString() },
    { _id: '2', title: 'Reported Post Flagged', message: 'User post flagged for commercial advertising spam.', priority: 'medium', read: false, createdAt: new Date().toISOString() },
    { _id: '3', title: 'Ecosystem Backup Done', message: 'Weekly automated cron backup executed successfully.', priority: 'low', read: true, createdAt: new Date(Date.now() - 3600000 * 24).toISOString() }
  ]);
  ...
  const getGreeting = () => {
    ...
    if (hrs < 12) return 'Good Morning, Administrator ☀️';
    if (hrs < 17) return 'Good Afternoon, Administrator ⛅';
    ...
    >
      ...
      <div className="flex-1 max-w-xs md:max-w-md mx-4 relative hidden md:block">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Global search ecosystems, logs, queries..."
            value={searchQuery}
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="hidden lg:flex items-center gap-1.5 border-r border-border/50 pr-3 mr-1">
          <Button
            variant="outline"
            size="sm"
            className="flex items-center gap-1 text-xs py-1.5 px-3 rounded-lg border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary"
            onClick={() => navigate('/admin/schemes')}
            title="Publish Government Scheme"
          >
          ...
          <Button
            variant="outline"
            size="sm"
            className="flex items-center gap-1 text-xs py-1.5 px-3 rounded-lg border-amber-500/20 bg-amber-500/5 hover:bg-amber-500/10 text-amber-400"
            onClick={() => navigate('/admin/experts')}
            title="Verify Advising Expert"
          >
                      ...
                      >
                        <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center text-xs border ${
                          notif.priority === 'high'
                            ...
                            : (notif.priority === 'medium' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-primary/10 text-primary border-primary/20')
                        }`}>
                          {notif.priority === 'high' ? <ShieldAlert className="w-4 h-4" /> : <Database className="w-4 h-4" />}
                        </div>
            ...
            {showProfileMenu && (
              ...
              >
                <div className="px-3.5 py-2.5 border-b border-border/50">
                  ...
                  <p className="text-[10px] text-text/40 truncate mt-0.5">{user?.email || 'admin@krishimitra.com'}</p>
                </div>
                <div className="py-1">
                  <Link
                    to="/admin/profile"
